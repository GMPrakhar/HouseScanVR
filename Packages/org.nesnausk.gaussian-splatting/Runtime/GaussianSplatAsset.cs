...
namespace GaussianSplatting.Runtime
{
    public class GaussianSplatAsset : ScriptableObject
    {
        ...
        //
        // The editor importer stores splat payloads in TextAsset blobs, which can
        // only be produced at asset-import time. A scan loaded from disk while the
        // game is running has no TextAssets, so it supplies raw byte buffers here
        // instead. Consumers must go through the Get*Data accessors below rather
        // than touching the TextAsset fields directly.
        // ---------------------------------------------------------------------
        NativeArray<byte> m_RtPos, m_RtOther, m_RtColor, m_RtSH, m_RtChunk;
        bool m_IsRuntime;

        public bool isRuntime => m_IsRuntime;

        public void SetRuntimeData(NativeArray<byte> pos, NativeArray<byte> other,
            NativeArray<byte> color, NativeArray<byte> sh, NativeArray<byte> chunk)
        {
            DisposeRuntimeData();
            m_RtPos = pos;
            m_RtOther = other;
        ...
        public void DisposeRuntimeData()
        {
            if (!m_IsRuntime)
                return;
            if (m_RtPos.IsCreated) m_RtPos.Dispose();
            if (m_RtOther.IsCreated) m_RtOther.Dispose();
            if (m_RtColor.IsCreated) m_RtColor.Dispose();
            if (m_RtSH.IsCreated) m_RtSH.Dispose();
            if (m_RtChunk.IsCreated) m_RtChunk.Dispose();
            m_IsRuntime = false;
        }
        ...
        public bool HasData =>
            m_IsRuntime
                ? m_RtPos.IsCreated && m_RtOther.IsCreated && m_RtColor.IsCreated && m_RtSH.IsCreated
                : m_PosData != null && m_OtherData != null && m_ColorData != null && m_SHData != null;

        public long posDataSize => m_IsRuntime ? m_RtPos.Length : m_PosData.dataSize;
        public long otherDataSize => m_IsRuntime ? m_RtOther.Length : m_OtherData.dataSize;
        public long colorDataSize => m_IsRuntime ? m_RtColor.Length : m_ColorData.dataSize;
        public long shDataSize => m_IsRuntime ? m_RtSH.Length : m_SHData.dataSize;
        public long chunkDataSize =>
            m_IsRuntime ? (m_RtChunk.IsCreated ? m_RtChunk.Length : 0)
                        : (m_ChunkData != null ? m_ChunkData.dataSize : 0);

        public NativeArray<T> GetPosData<T>() where T : struct =>
            m_IsRuntime ? m_RtPos.Reinterpret<T>(1) : m_PosData.GetData<T>();
        public NativeArray<T> GetOtherData<T>() where T : struct =>
            m_IsRuntime ? m_RtOther.Reinterpret<T>(1) : m_OtherData.GetData<T>();
        public NativeArray<T> GetColorData<T>() where T : struct =>
            m_IsRuntime ? m_RtColor.Reinterpret<T>(1) : m_ColorData.GetData<T>();
        public NativeArray<T> GetSHData<T>() where T : struct =>
            m_IsRuntime ? m_RtSH.Reinterpret<T>(1) : m_SHData.GetData<T>();
        public NativeArray<T> GetChunkData<T>() where T : struct =>
            m_IsRuntime ? m_RtChunk.Reinterpret<T>(1) : m_ChunkData.GetData<T>();
